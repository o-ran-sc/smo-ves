...
def test_listener_exp(mock_monitor, mock_input, body, start_response, schema):
    ...
    environ = {
        ...
        "HTTP_AUTHORIZATION": "Basic dXNlcjpwYXNzd29yZA==",
        "CONTENT_LENGTH": "2",
        "PATH_INFO": "/eventListener/v5/events",
    }
    body = {}
    ...
    dict_schema = {
        "v5": os.path.join(
            project_path,
            "collector/evel-test-collector/docs/att_interface_definition/CommonEventFormat-v7-2-1.json",
        )
    }
    try:
...
@mock.patch("gevent.pywsgi.Input", autospec=True)
@mock.patch("monitor.save_event_in_kafka")
def test_listener_command_list(
...
@mock.patch("gevent.pywsgi.Input", autospec=True)
@mock.patch("monitor.save_event_in_kafka")
def test_listener_command_list_none(
    mock_monitor, mock_input, body, start_response, schema, topic_name
):
    environ = {
        "REQUEST_METHOD": "POST",
