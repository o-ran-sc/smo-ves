...
def get_wrong_config_path():
    project_path = get_path()
    config_path = os.path.join(project_path, "tests/collector/wrong_config.conf")
    return config_path


def get_wrong_config_port_path():
    project_path = get_path()
    config_path = os.path.join(project_path, "tests/collector/port_config.conf")
    return config_path
...
@mock.patch("gevent.pywsgi.Input", autospec=True)
@mock.patch("monitor.save_event_in_kafka")
def test_b64_credientials_valid_or_not(mock_monitor, mock_input, body, start_response, schema):
    mock_input.__name__ = "read"
    environ = {
        "REQUEST_METHOD": "POST",
        "wsgi.input": mock_input,
...
@mock.patch(
    ...
    return_value=argparse.Namespace(
        verbose=None,
        api_version="7",
        config=get_wrong_config_port_path(),
        section="default",
    ),
)
@mock.patch("gevent.pywsgi.WSGIServer.serve_forever")
...
@mock.patch(
    "argparse.ArgumentParser.parse_args",
    return_value=argparse.Namespace(
        verbose=None, api_version="7", config=get_wrong_config_path(), section="default"
    ),
)
@mock.patch("gevent.pywsgi.WSGIServer.serve_forever")
...
def test_check_schema_file_contain_html(mock_sys, vel_schema_path, wrong_schema_ref):
    ...
    with pytest.raises(Exception):
        result = monitor.check_schema_file_exist(test_collector_path, wrong_schema_ref)
        assert result == os.path.join(
            path,
...
@patch("monitor.logger", logging.getLogger("monitor"))
@unittest.mock.patch("os.system")
def test_check_schema_file_contents_yaml(mock_sys, vel_schema_path, wrong_schema_ref2):
    """Test case for checking downloded schema ref file is yaml"""
    f = open(test_yaml_schema_file, "w+")
    f.write("NotifyNewAlarm")
    f.close()
    mock_sys.return_value = 0
    result = monitor.check_schema_file_exist(test_collector_path, wrong_schema_ref2)
